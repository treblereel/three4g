 ...
 *
 * @author Dmitrii Tikhomirov Created by treblereel on 2/27/18.
 */
...
public class WebGLRenderer {
  ...
  /**
   * The renderer obtains a RenderingContext context from its domElement by default, using
   * HTMLCanvasElement.getContext(). You can create this manually, however it must correspond to the
   * domElement in order to render to the screen.
   */
  public WebGLRenderingContext context;

  /**
   * A canvas where the renderer draws its output. This is automatically created by the renderer in
   * the constructor (if not provided already); you just need to add it to your page like so:
   * document.body.appendChild( renderer.domElement );
   */
  public HTMLElement domElement;

  /**
   * A wrapper for the .extensions.get method, used to check whether various WebGL extensions are
   * supported.
   */
  ...
  /**
   * If set, then it expects that all textures and colors need to be outputted in premultiplied
   ...
   * rendering process. Useful for debugging or just for the sake of curiosity. The object contains
   * the following fields: memory: geometries textures render: calls vertices faces points programs
   * By default these data are reset at each render calls, but when using the composer or mirrors it
   * can be preferred to reset them with a custom pattern : renderer.info.autoReset = false;
   * renderer.info.reset();
   */
  ...
  /**
   * Default is 8. The maximum number of MorphTargets allowed in a shader. Keep in mind that the
   * standard materials only allow 8 MorphTargets.
   */
  public int maxMorphTargets;

  /**
   * Default is 4. The maximum number of MorphNormals allowed in a shader. Keep in mind that the
   * standard materials only allow 4 MorphNormals.
   */
  public int maxMorphNormals;

  /**
   * Whether to use physically correct lighting mode. Default is false. See the lights / physical
   * example.
   */
  ...
  /**
   * Defines whether the renderer should sort objects. Default is true. Note: Sorting is used to
   * attempt to properly render objects that have some degree of transparency. By definition,
   * sorting objects may not work in all cases. Depending on the needs of application, it may be
   * neccessary to turn off sorting and use other methods to deal with transparency rendering e.g.
  ...
  /**
   * Attempt to allocate a texture unit for use by a shader. Will warn if trying to allocate more
   * texture units than the GPU supports. This is mainly used internally. See
   * capabilities.maxTextures.
   */
  public native void allocTextureUnit();

  /**
   * A build in function that can be used instead of requestAnimationFrame. For WebVR projects this
   * function must be used.
   *
   * @param callback — The function will be called every available frame. If `null` is passed it
   ...
   * initializes the color buffer to the current clear color value. Arguments default to true.
   */
   ...
   * @param color -- If set, then the color gets cleared. This method clears a rendertarget. To do
   *     this, it activates the rendertarget.
   */
   ...
   * @param depth -- If set, then the depth gets cleared. This method clears a rendertarget. To do
   *     this, it activates the rendertarget.
   */
   ...
   * @param stencil -- If set, then the stencil gets cleared. This method clears a rendertarget. To
   *     do this, it activates the rendertarget.
   */
  ...
  /**
   * Copies all pixels of a texture to an existing texture starting from the given position. Enables
   * access to WebGLRenderingContext.texSubImage2D.
   *
   * @param position instance of Vector2
  ...
  public native void copyTextureToTexture(
      Vector2 position, Texture srcTexture, Texture dstTexture, int level);

  /** Dispose of the current rendering context. */
  public native void dispose();

  /**
   * Simulate loss of the WebGL context. This requires support for the WEBGL_lose_context
   * extensions. According to WebGLStats, as of February 2016 90% of WebGL enabled devices support
   * this.
   */
  ...
  public native void setRenderTarget(
      WebGLRenderTarget renderTarget, int activeCubeFace, int activeMipMapLevel);
  ...
  /**
   * Reads the pixel data from the renderTarget into the buffer you pass in. Buffer should be a
   * Javascript Uint8Array instantiated with new Uint8Array( renderTargetWidth * renderTargetWidth *
   * 4 ) to account for size and color information. This is a wrapper around gl.readPixels. See the
   * interactive / cubes / gpu example.
   *
  ...
  /**
   * Render a scene using a camera. The render is done to the renderTarget (if specified) or to the
   * canvas as usual. If forceClear is true, the depth, stencil and color buffers will be cleared
   * before rendering even if the renderer's autoClear property is false. Even with forceClear set
   * to true you can prevent certain buffers being cleared by setting either the autoClearColor,
  ...
  /**
   * TODO: Render a buffer geometry group using the camera and with the specified material.
   *
   * @param camera instance of Camera
   ...
   */
  public native void renderBufferImmediate(
      Object3D object, WebGLProgram shaderprogram, Material shading);
  ...
  /**
   * This method sets the active rendertarget. If the parameter is omitted the canvas is set as the
   * active rendertarget.
   */
  public native void setRenderTarget();
   ...
   *
   * @param target — the result will be copied into this Vector4.
   * @return Returns the scissor region.
   */
   ...
   *
   * @return true if scissor test is enabled; returns false otherwise.
   */
  public native boolean getScissorTest(Vector4 target);
   ...
   * @param height as int
   */
  public native void setScissor(float x, float y, float width, float height);

  /**
   * Sets the scissor area from (x, y) to (x + width, y + height)
   *
   * @param target -- a 4-component vector specifying the parameters of the region.
   */
  public native void setScissor(Vector4 target);
  ...
  /**
   * Return a Boolean true if the context supports vertex textures. This has been deprecated in
   * favour of capabilities.vertexTexures.
   *
  ...
  /**
   * Resizes the output canvas to (width, height) with device pixel ratio taken into account, and
   * also sets the viewport to fit that size, starting in (0, 0). Setting updateStyle to false
   * prevents any style changes to the output canvas.
   *
  ...
  /**
   * This method sets the correct texture to the correct slot for the WebGL shader. The slot number
   * can be found as a value of the uniform of the sampler.
   *
   * <p>Note: This method replaces the older # .setTexture method.
   *
   ...
   */
  public native void setViewport(float x, float y, float width, float height);

  /**
   * Sets the viewport to render from (x, y) to (x + width, y + height).
   *
   * @param vector a 4-component vector specifying the parameters of a viewport.
   */
  public native void setViewport(Vector4 vector);
}
